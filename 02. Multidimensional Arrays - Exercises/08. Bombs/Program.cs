        ...
        private static void Explosion(Queue<string> coordinate)
        {
            while (coordinate.Count > 0)
            {
                ...
                if (matrix[r][c] > 0)
                {
                    ...
                    if (IsToExplode(r - 1, c - 1))
                    {
                        matrix[r - 1][c - 1] -= bombPower;
                    }
                    if (IsToExplode(r - 1, c + 1))
                    {
                        matrix[r - 1][c + 1] -= bombPower;
                    }
                    if (IsToExplode(r, c - 1))
                    {
                        matrix[r][c - 1] -= bombPower;
                    }
                    if (IsToExplode(r, c + 1))
                    {
                        matrix[r][c + 1] -= bombPower;
                    ...
                    if (IsToExplode(r - 1, c))
                    {
                        matrix[r - 1][c] -= bombPower;
                    }
                    if (IsToExplode(r + 1, c))
                    {
                        matrix[r + 1][c] -= bombPower;
